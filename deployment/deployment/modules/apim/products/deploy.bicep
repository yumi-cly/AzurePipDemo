...
@description('Optional. Array of Policy Groups.')
...
module product_policies 'policies/deploy.bicep' = [for (policy, index) in policies: {
  name: '${deployment().name}-Apim-Policy-${index}'
  params: {
    apiManagementServiceName: apiManagementServiceName
    name: policy.name
    productName: name
